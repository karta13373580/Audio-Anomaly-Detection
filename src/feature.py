...
from .resnext101 import resnext_wsl
# backbone nets
#backbone_nets = {'vgg11': VGG11, 'vgg13': VGG13, 'vgg16': VGG16, 'vgg19': VGG19}
backbone_nets = {'resnext_wsl': resnext_wsl}
...
class AvgFeatAGG2d(nn.Module):
    ...
    def forward(self, input):
        ...
        output = torch.reshape(output, (N, C, int(self.output_size[0]*self.output_size[1])))
        # print(output.shape)
...
class Extractor(nn.Module):
    ...
    def __init__(self, backbone='resnext_wsl',
                 cnn_layers=("relu1_1",),
                 upsample="nearest",
                 is_agg=True,
                 kernel_size=(4, 4),
        ...
        self.device = device
        # feature processing
        padding_h = (self.patch_size[0] - self.stride[0]) // 2 #0
        padding_w = (self.patch_size[1] - self.stride[1]) // 2 #0
        ...
        # self.out_w: 64
        self.out_size = (self.map_size[0], self.map_size[1])
        # print(self.out_size)
    ...
    def forward(self, input):
        ...
        for _, feat_map in feat_maps.items():
            if self.is_agg:
                ...
                # print(feat_map.shape) #torch.Size([32, channel, 64, 64])
                # # aggregating features for every pixel

                feat_map = self.feat_agg(feat_map) #經過這層變4096
                # concatenating
...
#     time_s = time.time()
#     batch_size = 1
#     input = torch.Tensor(np.random.randn(batch_size, 3, 256, 256))
